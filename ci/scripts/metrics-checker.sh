...
checkStatefulSetYAML(){
    SERVICE_NAME=$1
    if grep -q "{{- include \"$SERVICE_NAME.annotations\" . | nindent [0-9] }}" ./charts/$SERVICE_NAME/templates/statefulSet.yaml &&
       grep -q "\"$SERVICE_NAME.prometheus\"" ./charts/$SERVICE_NAME/templates/_helpers.tpl; then
        ...
        echo -e "What is needed:"
        echo -e "{{- include \"$SERVICE_NAME.annotations\" . | nindent [0-9] }} in statefulSet.yaml"
